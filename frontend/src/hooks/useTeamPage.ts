...
import { InterviewTeamType, TeamSubmitType } from 'types/team';

const useTeamPage = () => {
  const { isDebounce } = useUtil();
  const { participants, setParticipants, getTeam, postTeam, editTeam, requestGetMembers } =
    useTeam();
  ...
  const onSubmitTeamAddForm = ({ watchers, participants }: TeamSubmitType) => {
    ...
    const teamInfo = {
      ...
      watchers: {
        ids: Object.values(watchers).map((watcher) => watcher.id),
      },
      participants: {
        ids: Object.values(participants).map((participant) => participant.id),
      },
    };
    postTeam({ teamInfo });
  };
  ...
  const handleClickTeamAddButton = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    onSubmitTeamAddForm({ watchers, participants });
  };
