...
public class Team extends BaseEntity {
    ...
    private void validateTitle(final String title) {
        if (title == null) {
            throw new InvalidFieldException("팀 이름이 없습니다.");
        }
        if (title.isEmpty()) {
            throw new InvalidFieldException("팀 이름이 비어있습니다.");
        }
        if (title.length() > DEFAULT_STRING_SIZE) {
            throw new InvalidFieldException("잘못된 팀 이름을 입력했습니다.");
        }
    }

    private void validatePlace(final String place) {
        if (place == null) {
            throw new InvalidFieldException("장소가 없습니다.");
        }
        if (place.isEmpty()) {
            throw new InvalidFieldException("장소가 비어있습니다.");
        }
        if (place.length() > DEFAULT_STRING_SIZE) {
            throw new InvalidFieldException("잘못된 장소를 입력했습니다.");
        }
    }
    ...
    private void validateStartAt(final LocalDateTime startAt) {
        ...
        if (LocalDateTime.now().isAfter(startAt)) {
            throw new InvalidFieldException("잘못된 시작 시간을 입력했습니다.");
        }
    }

    private void validateProfileUrl(final String profileUrl) {
        if (profileUrl == null) {
            throw new InvalidFieldException("팀 프로필 사진이 없습니다.");
        }
        if (profileUrl.isEmpty()) {
            throw new InvalidFieldException("팀 프로필 사진이 비어있습니다.");
        }
        if (profileUrl.length() > PROFILE_URL_SIZE) {
            throw new InvalidFieldException("잘못된 팀 프로필 사진을 입력했습니다.");
        }
    }
