...
operation::team/update/exception/participants/duplicate[]

[[update-exception-participants-host]]
==== 수정 시 참가자 목록에 Host가 포함된 경우 (hostId = 1)

operation::team/update/exception/participants/host[]

[[update-exception-participants-empty]]
==== 수정 시 호스트 이외의 참가자가 없는 경우

operation::team/update/exception/participants/empty[]

[[update-exception-participants-null]]
==== 수정 시 participants가 null인 경우

operation::team/update/exception/participants/null[]

[[update-exception-interviewer-number-not-positive]]
==== 수정 시 인터뷰어 수가 1미만인 경우

operation::team/update/exception/interviewer-number/not-positive[]

[[update-exception-interviewer-number-not-positive]]
==== 수정 시 인터뷰어 수가 참가자 수 보다 많거나 같은 경우

operation::team/update/exception/interviewer-number/more-than-participant[]
...
==== 호스트가 아닌 사용자가 인터뷰를 삭제하려는 경우

include::{snippets}/team/delete/exception/unauthorized/http-response.adoc[]

[[delete-exception-alreadyClose]]
==== 이미 종료된 인터뷰를 삭제하려는 경우

include::{snippets}/team/delete/exception/already-close/http-response.adoc[]
