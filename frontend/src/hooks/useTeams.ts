  ...
  const handleClickFilterButtons = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    switch (eventTarget.innerText) {
      case '진행중인 인터뷰':
        ...
        await setTeamsCondition({ open: true, close: false, my: false });
        break;
      case '종료된 인터뷰':
        if (teamsCondition.close === true) return;

        await setTeamsCondition({ open: false, close: true, my: false });
        break;
      case '나의 인터뷰':
        if (teamsCondition.my) return;

        await setTeamsCondition({ open: false, close: false, my: true });
        break;
    }
  };
