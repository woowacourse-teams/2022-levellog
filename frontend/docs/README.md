...
- [x] 팀에 속해 있을 때만 접속할 수 있는 페이지에 대한 인증&인가를 구현한다.
- [x] 팀의 호스트일 때만 접속할 수 있는 페이지에 대한 인증&인가를 구현한다.
- [x] 본인만 접속할 수 있는 페이지에 대한 인증&인가를 구현한다.
- [x] 본인이 아닐때만 접속할 수 있는 페이지에 대한 인증&인가를 구현한다.
- [x] 인증/인가를 완료한 후, 팀 상태에 관한 체크를 한다.
- [x] 로그인 버그를 해결한다.
  - [x] 가끔 로그인이 되어도 로그인 페이지에 계속 남아있는 버그를 해결한다.
  - [x] url 입력으로 페이지를 이동할 때 로그아웃이 되는 버그를 해결한다.
  - [ ] 첫 진입시 "유효하지 않은 토큰입니다" alert이 무한으로 뜬다.
- [x] 엣지 케이스를 체크한다.
- [ ] 첫 진입이 `Home`이라 바로 Detail페이지로 넘어가지 않음.
